<template>
            ...
            <section class="main-content">
                <div class="headerTable">
                    ...
                    <div class="search-bar">
                        ...
                        <button @click="crearDispositivos" class="icon-button">
                ...
                <div class="table-container">
                    ...
                    <table class="patient-table" v-if="!IsLoading">
                        ...
                        <tbody class="tableBody">
                            <tr v-for="dispositivo in listaDispositivo" :key="dispositivo.idDispositivo">
                                <td>{{ dispositivo.idDispositivo }}</td>
                                <td>{{ nombrepersonal(dispositivo.idPersonal) }}</td>
                                <td>{{ dispositivo.numDispositivo }}</td>
                                <td>{{ dispositivo.observacionDispositivo }}</td>
                                <td>{{ dispositivo.estadoDispositivo }}</td>
                                <td>
                                    <button @click="editarDispositivo(dispositivo.idDispositivo)" class="edit-button">
                                        <!-- Botón de editar -->
                                    ...
                                    <button @click="eliminarDispositivo(dispositivo.idDispositivo)" class="delete-button">
                                        <!-- Botón de eliminar -->
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                            viewBox="0 0 24 24">
    ...
    <!-- Modales -->
    <CrearModal v-if="modalCrearShow" @actualizarDispositivo="listarDispositivo"></CrearModal>
    <EditarModal v-if="modalEditarShow" :idDispositivo="idDispositivo" :listaPersonal="listaPersonal" @actualizarDispositivo="listarDispositivo"></EditarModal>
    <EliminarModal v-if="modalEliminarShow" :idDispositivo="idDispositivo" @actualizarDispositivo="listarDispositivo"></EliminarModal>
...
import CrearModal from '@/components/Dispositivos/CrearModal.vue';
import bootstrap from 'bootstrap/dist/js/bootstrap.bundle.min.js';
import EditarModal from '@/components/Dispositivos/EditarModal.vue';
import EliminarModal from '@/components/Dispositivos/EliminarModal.vue';


let listaDispositivo = ref([]);
let listaPersonal = ref([]);
let IsLoading = ref(false);
let modalCrearShow = ref(false);
let modalEditarShow = ref(false);
let idDispositivo = ref(0);
...
function listarDispositivo() {
    ...
    axiosFunction.get("Dispositivos/ListarDispositivos")
        .then(resultado => {
            listaDispositivo.value = resultado.data.resultado;
            return axiosFunction.get("Personal/ListarPersonal");
        })
        .then(personalResultado => {
            listaPersonal.value = personalResultado.data.resultado;
...
function nombrepersonal(idPersonal) {
    const personalEncontrado = listaPersonal.value.find(p => p.idPersonal === idPersonal);
    return personalEncontrado ? personalEncontrado.nombrePersonal : 'Sin personal asignado';}
